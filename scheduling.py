    ...
    for m in range(nr_employees):
        ...
        for t in range(nr_days):
            ...
            if start is not None and end is not None:
                start = datetime.datetime(date_datetime.year, date_datetime.month, date_datetime.day, start.hour,
                                          start.minute)
...
def writeToExcel(plan, weeklyHours, demand_vs_planned):
    ...
    with pd.ExcelWriter("Solution.xlsx", engine='xlsxwriter') as writer:
        ...
        worksheet_weeklyHours.conditional_format('E2:E10000',
                                                 {"type": "cell", "criteria": ">", "value": 0, "format": green_format})
        worksheet_weeklyHours.conditional_format('F2:F10000',
...
x = [[[model.add_var(name="x_" + list_employees[m] + "_" + days[t] + "_" + str(slots[s]), var_type=BINARY) for s in
       range(nr_slots)] for t in range(nr_days)] for m in range(nr_employees)]

# boolean decision variable: z[m][t][s]: employee m starts working day on day t in slot s
z = [[[model.add_var(name="z_" + list_employees[m] + "_" + days[t] + "_" + str(slots[s]), var_type=BINARY) for s in
       range(nr_slots)] for t in range(nr_days)] for m in range(nr_employees)]

# boolean auxiliary variable: y[m][t]: employee m works on day t (at least one slot)
...
for m in range(nr_employees):
    for t in range(nr_days):
        # y[m][t] = 1 if any x[m][t][s] = 1
        model += y[m][t] <= xsum(x[m][t][s] for s in range(nr_slots)), "link_y_upper_" + list_employees[m] + "_" + days[
            t]
        model += y[m][t] * nr_slots >= xsum(x[m][t][s] for s in range(nr_slots)), "link_y_lower_" + list_employees[
            m] + "_" + days[t]
...
if parameters.loc["demand_can_open_store"]["to consider"] == "yes":
    demand_can_open_store = float(parameters.loc["demand_can_open_store"]["Value"])
    for t in range(nr_days):
        constraint_name = "constraint_can_open_store_" + days[t] + "_" + str(slots[0])
        model += xsum(data_employees.loc[list_employees[m], "can_open_store"] * x[m][t][0]
                      for m in range(nr_employees)
                      ) >= demand_can_open_store, constraint_name

# optional constraint: Demand for close store in first slot
if parameters.loc["demand_can_close_store"]["to consider"] == "yes":
    demand_can_close_store = float(parameters.loc["demand_can_close_store"]["Value"])
    for t in range(nr_days):
        constraint_name = "constraint_can_close_store_" + days[t] + "_" + str(slots[-1])
        model += xsum(data_employees.loc[list_employees[m], "can_close_store"] * x[m][t][-1]
                      for m in range(nr_employees)
                      ) >= demand_can_close_store, constraint_name
...
if parameters.loc["max_hours_per_week"]["to consider"] == "yes":
    for m in range(nr_employees):
        ...
        days_in_a_week = 7
        for t in range(nr_days):
            begin_of_week = t // days_in_a_week
            model += xsum(x[m][t][s]
                          for d in range(begin_of_week, begin_of_week + days_in_a_week)
                          for s in range(nr_slots)) <= (
                             hoursPerWeek + overtime) * 2, constraint_name
...
if parameters.loc["min_workingTime_per_Day"]["to consider"] == "yes":
    for m in range(nr_employees):
        for t in range(nr_days):
            ...
            rhs = 2 * float(parameters.loc["min_workingTime_per_Day"]["Value"]) * (
                xsum(z[m][t][s] for s in range(nr_slots)))
            model += xsum(x[m][t][s] for s in range(nr_slots)) >= rhs, constraint_name
...
if parameters.loc["min_hours_per_week"]["to consider"] == "yes":
    for m in range(nr_employees):
        ...
        model += xsum(x[m][t][s] for t in range(nr_days) for s in range(nr_slots)) >= (
                hoursPerWeek - minusHours) * 2, constraint_name
